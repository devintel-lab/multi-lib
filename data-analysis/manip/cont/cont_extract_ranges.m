...
for range_idx = 1:size(ranges, 1)
    ...
    % These two arrays are logical indicies for var_data.
    ge_start = range(1) <= var_data(:, 1) + float_tolerance;
    lt_end = var_data(:, 1) + float_tolerance < range(2);
    
    sections{range_idx} = var_data(ge_start & lt_end, :);
end
