...
if isempty(chunks)
    warning('The input CHUNKS is empty, there is no data inside, the function will return now');
    results = [];
    return
end
...
%     if ~strcmp(get_data_type(input.var_name), 'cevent')
...
elseif strcmp(grouping, 'subject') && isfield(input, 'sub_list') ...
    ...
    individual_range_dur = vertcat(chunks_trial_time{:});
    input.individual_range_dur = individual_range_dur;
elseif strcmp(grouping, 'trial_cat') && isfield(input, 'sub_list') ...
        && length(chunks) == length(input.sub_list)
    chunks_trial_time = arrayfun(@(sub_id) ...
        event_total_length(get_trial_times(sub_id)), ...
        input.sub_list, ...
    ...
    individual_range_dur = vertcat(chunks_trial_time{:});
    input.individual_range_dur = individual_range_dur;
elseif strcmp(grouping, 'trial') && isfield(input, 'sub_list') ...
        && (~isfield(input, 'trial_indices') && ~isfield(input, 'trial_var_name'))
    chunks_trial_time = arrayfun(@(sub_id) ...
        get_trial_times(sub_id), ...
        input.sub_list, ...
        'UniformOutput', false);
...
elseif strcmp(grouping, 'trial') && isfield(input, 'sub_list') ...
    ...
    for sidx = 1:length(sub_list)
        ...
        if iscell(input.trial_indices)
            trial_indices_one = input.trial_indices{sidx};
        else
            trial_indices_one = input.trial_indices;
        end
...
for chunkidx = 1:length(chunks)
    ...
    if has_transition_matrix
        if isempty(chunk_one)
            ...
            if isfield(input, 'trans_max_gap')        
                trans_matrix_one = cevent_transition_matrix(chunk_one, ...
                    input.trans_max_gap, max_category);
            else
                trans_matrix_one = cevent_transition_matrix(chunk_one, ...
                    Inf, max_category);
            end
...
% hist_bins
if isfield(input, 'hist_arg')
    hist_arg = input.hist_arg;
else
    hist_arg = 'thresholds';
end
...
if isfield(input, 'hist_bins')
    ...
    for i = 1: length(chunks)
        res_dur_hist(i,:) = event_duration_hist(chunks{i}, input.hist_bins, hist_arg);
        res_dur_hist(i,:) = res_dur_hist(i,:)/sum(res_dur_hist(i,:));
    end
       
    results.dur_hist = event_duration_hist(cat_chunks, input.hist_bins, hist_arg);
