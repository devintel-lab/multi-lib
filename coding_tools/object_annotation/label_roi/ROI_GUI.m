...
if isempty(coder_name)
	existing_data = [subject_dir '/extra_p/' subject '_ROI.mat'];
else
	existing_data = [subject_dir '/extra_p/' subject '_ROI_' coder_name '.mat'];
end
...
% imshow(img_view, 'Parent', handles.frame_1);
% imshow(img_eye, 'Parent', handles.frame_2);
% imshow(img_third, 'Parent', handles.frame_3);
% imshow(img_parent, 'Parent', handles.frame_4);
...
subject_dir = getappdata(handles.frame_1, 'SUBJECT_DIR');
cstream_roi = data_to_cstream(ROI_DATA);
if isempty(coder_name)
	save([subject_dir '/extra_p/' subject_name '_ROI.mat'], 'ROI_DATA');
  %save([subject_dir '/derived/cstream_eye_roi_child.mat'], 'cstream_roi');
  record_variable(get_subject_id_from_folder(handles), 'cstream_eye_roi_child', cstream_roi);
else
	save([subject_dir '/extra_p/' subject_name '_ROI_' coder_name '.mat'], 'ROI_DATA');
  % save([subject_dir '/extra_p/cstream_eye_roi_child_' coder_name '.mat'], 'cstream_roi');
  record_variable_into_specified_directory(get_subject_id_from_folder(handles), 'extra_p', ['cstream_eye_roi_child_' coder_name], cstream_roi);
end
...
last_coded = max(find([ROI_DATA(:).is_coded] == 1));
if last_coded == 0
	last_coded = 1;
end
...
function [cstream_roi] = data_to_cstream(ROI_DATA)
is_coded = [ROI_DATA(:).is_coded] == 1;
index = 1:length(find(is_coded));
time = (index-1) * 1/30 + 30;
roi = [ROI_DATA(is_coded).ROI];
cstream_roi = [time' roi'];
