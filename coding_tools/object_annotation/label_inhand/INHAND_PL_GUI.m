function varargout = INHAND_PL_GUI(varargin)
% INHAND_PL_GUI MATLAB code for INHAND_PL_GUI.fig
%      INHAND_PL_GUI, by itself, creates a new INHAND_PL_GUI or raises the existing
%      singleton*.
%
%      H = INHAND_PL_GUI returns the handle to a new INHAND_PL_GUI or the handle to
%      the existing singleton*.
%
%      INHAND_PL_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in INHAND_PL_GUI.M with the given input arguments.
%
%      INHAND_PL_GUI('Property','Value',...) creates a new INHAND_PL_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before INHAND_PL_GUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to INHAND_PL_GUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help INHAND_PL_GUI
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @INHAND_PL_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @INHAND_PL_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before INHAND_PL_GUI is made visible.
function INHAND_PL_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to INHAND_PL_GUI (see VARARGIN)

% Choose default command line output for INHAND_PL_GUI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes INHAND_PL_GUI wait for user response (see UIRESUME)
...
function varargout = INHAND_PL_GUI_OutputFcn(hObject, eventdata, handles) 
...
if isempty(coder_name)
	existing_data = [subject_dir '/extra_p/' subject '_PL_INHAND.mat'];
else
	existing_data = [subject_dir '/extra_p/' subject '_PL_INHAND_' coder_name '.mat'];
end
...
set(s,'CData', img_parent);
set(v,'CData', img_child);
...
if isempty(coder_name)
	save([subject_dir '/extra_p/' subject_name '_PL_INHAND.mat'], 'INHAND_DATA');
  % save([subject_dir '/derived/cstream_inhand_parent_left.mat'], 'cstream_inhand');
  record_variable(get_subject_id_from_folder(handles), 'cstream_inhand_parent_left', cstream_inhand);
else
	save([subject_dir '/extra_p/' subject_name '_PL_INHAND_' coder_name '.mat'], 'INHAND_DATA');
  % save([subject_dir '/extra_p/cstream_inhand_parent_left_' coder_name '.mat'], 'cstream_inhand');
  record_variable_into_specified_directory(get_subject_id_from_folder(handles), 'extra_p', ['cstream_inhand_parent_left_' coder_name], cstream_inhand);
end
...
s = imshow(img_parent, 'Parent', handles.frame_1);
v = imshow(img_child, 'Parent', handles.frame_2);
