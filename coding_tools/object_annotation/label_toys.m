function varargout = label_toys(varargin)
    ...
    % Last Modified by GUIDE v2.5 01-Sep-2015 12:07:40
...
function varargout = label_toys_OutputFcn(hObject, eventdata, handles) 
    ...
    % create toy list
    toys = {'helmet', 'house', 'blue car', 'flower', 'elephant', 'snowman',...
        'rabbit', 'spongebob', 'turtle', 'gavel', 'bug', 'mantis', ...
        'green car', 'saw', 'doll', 'phone', 'rubik''s cube', 'rake', ...
        'truck', 'white/blue car', 'ladybug', 'purple cube', 'bed', ...
        'ball in cube'};
...
function load_button_Callback(hObject, eventdata, handles)
    ...
    [file_name, file_dir] = uigetfile(fullfile(filesep, 'cantor', 'space', 'experiment_11', 'frame_lists', '*.txt'), 'Select a text file that contains a list of frames');
...
function box_button_Callback(hObject, eventdata, handles)
    ...
    setColor(h, colors(toy_id, :));
    pos = getPosition(h);
    % make sure box is within image;
    pos = crop_box_to_image(pos);
...
function [] = render_image_with_boxes(handles)
    ...
    for b = 1:size(annotation_data(index).boxes, 1)
        if ~isnan(annotation_data(index).boxes(b, 1))
            ...
            text = text2im(toys{b});
            text = cat(3, text, text, text)*255;
            text = imresize(text, 0.5);
            img = implace(img, text, round(pos(2)), round(pos(1)));
        end
    end
end

function box_out = crop_box_to_image(box_in)
