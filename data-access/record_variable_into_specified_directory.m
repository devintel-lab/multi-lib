...
function [ filename ] = record_variable_into_specified_directory( subject_id, sub_directory, variable_name, data )
%Records the given data under the given subject's sub directory and name.
%   record_variablein_specified_directory(SUBJECT_ID, SUB_DIRECTORY, VARIABLE_NAME, DATA)
%
...
%
    ...
    var_dir = [get_subject_dir(subject_id) filesep() sub_directory];
    if exist(var_dir, 'dir')==0
        mkdir(var_dir);
    end
    filename = [var_dir filesep() variable_name '.mat'];
