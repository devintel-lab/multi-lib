function filter_rescale_cont_eye_vars(sid)
...
if has_variable(sid, 'cont2_eye_xy_child')
    ...
    if max(cont2_child_xy(:,2)) <= 640
        % x bounds
        cont2_child_xy(cont2_child_xy(:,2) > 640, 2:3) = NaN;
        cont2_child_xy(cont2_child_xy(:,2) < 0, 2:3) = NaN;
        
        fprintf('\nmax value of cont2_eye_x child <= 640 %d, now filter and rescale to 720\n', sid);
        cont2_child_xy(:,2) = cont2_child_xy(:,2)/640 * 720; % HARD CODING, mapping
    else
...
if has_variable(sid, 'cont2_eye_xy_parent') && exp_id ~= 29
    % get cont2_parent_eye_xy and filter
    cont2_parent_xy = get_variable(sid, 'cont2_eye_xy_parent');

    if max(cont2_parent_xy(:,2)) <= 640      
        % x bounds
        cont2_parent_xy(cont2_parent_xy(:,2) > 720, 2:3) = NaN;
        cont2_parent_xy(cont2_parent_xy(:,2) < 0, 2:3) = NaN;
        
        fprintf('\ncont2_eye_x parent <= 640 %d, now filter and rescale to 720\n', sid);
        cont2_parent_xy(:,2) = cont2_parent_xy(:,2)/640 * 720; % HARD CODING, mapping
    else
        cont2_parent_xy(cont2_parent_xy(:,2) > 720, 2:3) = NaN;
        cont2_parent_xy(cont2_parent_xy(:,2) < 0, 2:3) = NaN;            
    end
