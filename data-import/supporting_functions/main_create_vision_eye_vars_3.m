function main_create_vision_eye_vars_3(sub_id, target_cam, dest_cam, agent_type, obj_num, obj_params, is_record_vision_vars, is_record_eye_vars, seg_image_overwrite_flag, seg_image_format, img_step)
...
if ~exist('is_record_eye_vars', 'var')
    is_record_eye_vars = 0;
end
    
if ~exist('is_record_vision_vars', 'var')
    is_record_vision_vars = 0;
end
...
[time, blob_size_all, blob_center_all, blob_dyn_all, eye_mean_dist, eye_min_dist, center_mean_dist, center_min_dist] = ...
    main_detect_objects_eye_dist_3(sub_id, jpg_folder, output_folder, agent_type, obj_num, obj_params, is_record_eye_vars, seg_image_overwrite_flag, seg_image_format, img_step);
...
%     record_variable(sub_id, ['cont_vision_dyn_skin_' agent_type ], [time blob_size_all(:,1)*100]);
 ...
 %         record_variable(sub_id, 'cont_gaze_to_head_mean_distance', [time mean_dist(:,1)]);
        ...
        record_variable(sub_id, ['cont_vision_mean-dist_center-to-all_' agent_type], [time center_mean_dist(:,5)]);
%         record_variable(sub_id, 'cont_' agent_type '_gaze_to_head_min_distance', [time center_min_dist(:,1)]);
        ...
        record_variable(sub_id, ['cont_vision_min-dist_center-to-all_' agent_type], [time center_min_dist(:,5)]);
    end
...
%         record_variable(sub_id, 'cont_gaze_to_head_mean_distance', [time mean_dist(:,1)]);
    record_variable(sub_id, ['cont_eye-vision_mean-dist_gaze-to-obj1_' agent_type], [time eye_mean_dist(:,2)]);
    record_variable(sub_id, ['cont_eye-vision_mean-dist_gaze-to-obj2_' agent_type], [time eye_mean_dist(:,3)]);
    record_variable(sub_id, ['cont_eye-vision_mean-dist_gaze-to-obj3_' agent_type], [time eye_mean_dist(:,4)]);
    record_variable(sub_id, ['cont_eye-vision_mean-dist_gaze-to-all_' agent_type], [time eye_mean_dist(:,5)]);

%         record_variable(sub_id, 'cont_' agent_type '_gaze_to_head_min_distance', [time min_dist(:,1)]);
    record_variable(sub_id, ['cont_eye-vision_min-dist_gaze-to-obj1_' agent_type], [time eye_min_dist(:,2)]);
    record_variable(sub_id, ['cont_eye-vision_min-dist_gaze-to-obj2_' agent_type], [time eye_min_dist(:,3)]);
    record_variable(sub_id, ['cont_eye-vision_min-dist_gaze-to-obj3_' agent_type], [time eye_min_dist(:,4)]);
    record_variable(sub_id, ['cont_eye-vision_min-dist_gaze-to-all_' agent_type], [time eye_min_dist(:,5)]);
end
